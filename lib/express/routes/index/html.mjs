...
import fs from 'fs';
import { getEnv, isQueryStringTruthy } from '../../../utils';
import {
    getAppName,
    getAppDescription,
    getBaseUrl,
} from '../../../metadata';
import { getStatusForClient, isClientInQueue } from '../../../queue';
...
const SVG_IMAGE = fs.readFileSync(path.join(PUBLIC_DIR, 'images/toilet.svg')).toString();
...
export default async (clientId, req, res) => {
    ...
    if (isQueryStringTruthy(req.query.dashboard))
    {
    ...
    else if (isClientInQueue(clientId))
    {
        thisIndex = thisIndex.replace(
            '<button id=subscribe',
